...
export const CategoryPage = () => {
  ...
  // Фильтрация постов: исключаем свои
  const filteredPosts = category.posts?.filter(post => post.userId !== currentUser?.id) ?? [];

  return (
    <div className="lg:col-start-2 lg:max-w-2xl">
      <h1 className="mb-8 text-center text-4xl font-bold">{category.name}</h1>
      <p className="text-base text-slate-700 sm:text-lg text-center">{category.description}</p>
